...
int PyOP() {
	...
	//{
		...
		{
			userWantsToExit = userOutputClass.display(datumsPtr);;
			userOutputClass.printKeypoints(datumsPtr);
		}
		else
...
std::shared_ptr<std::vector<op::Datum>> initialize(int FLAGS_logging_level, std::string FLAGS_output_resolution,
	...
	datumsPtr->at(0).name = "test";
	std::cout << "INIT : " << datumsPtr->at(0).name << std::endl;*/
	return datumsPtr;
}

op::Datum getDatum() {
	return datumsPtr->at(0);
}
