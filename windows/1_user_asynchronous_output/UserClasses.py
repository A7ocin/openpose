...
class UserInputClass:
    ...
    def createDatum(self, datumsPtr, image, resolution):
        ...
        if (self.mClosed or len(self.mImageFiles) <= self.mCounter):
		    ...
		    # Create new datum
            PyOP.emplaceBack(datumsPtr)
            #self.datum = PyOP.datum_frompointer(PyOP.datumsPtr_at(datumsPtr)).__getitem__(0)
            # Fill datum
        ...
        for w in range(width):
            for h in range(height):
                for c in range(channels):
                    PyOP.setElement(h, w, c, datumsPtr, int(np_image[h][w][c]), width, height)
...
class UserOutputClass:
    def init(self):
        return PyOP.UserOutputClass()

    def getProcessedImage(self, datumsPtr, resolution):
        if (datumsPtr and not PyOP.datumsPtr_empty(datumsPtr)):
            width = int(resolution.split('x')[0])
            height = int(resolution.split('x')[1])
            channels = 3
            ...
            for w in range(width):
                for h in range(height):
                    for c in range(channels):
                        np_array[h][w][c] = PyOP.getElement(h,w,c,datumsPtr)
            return np_array
        else:
            print "Nullptr or empty datumsPtr found."

def getDatum(datumsPtr):
    return PyOP.datum_frompointer(PyOP.datumsPtr_at(datumsPtr)).__getitem__(0)
